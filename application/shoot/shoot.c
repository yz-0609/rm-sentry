...
void ShootInit()
{
    ...
    friction_config.can_init_config.can_handle=&hcan2;
...
void ShootTask()
{
    ...
    else // 恢复运行
    {
        ...
        if(shoot_cmd_recv.friction_mode==FRICTION_ON)
        {
            DJIMotorEnable(friction_l);
            DJIMotorEnable(friction_r);
            DJIMotorSetRef(friction_l,30000.0f);    //33500
            DJIMotorSetRef(friction_r,30000.0f);
            target=18000.0f;
        }
    ...
    ref=-friction_l->measure.speed_aps;
    erro=friction_l->measure.speed_aps-friction_r->measure.speed_aps;
