...
DJIMotorInstance *DJIMotorInit(Motor_Init_Config_s *config)
{
    ...
    // motor basic setting 电机基本设置  1.类型  2.正反转标志，闭环类型，数据反馈源类型
    ...
    // 注册电机到CAN总线
    config->can_init_config.can_module_callback = DecodeDJIMotor; // 设置CAN回调函数
...
//动态改变电机数据的反馈源，速度和角度可变为陀螺仪和编码器反馈
...
void DJIMotorControl()
{
    ...
    { // 减小访存开销,先保存指针引用
        ...
        // 分组填入发送数据
        group = motor->sender_group;   //在MotorSenderGrouping函数被设置
